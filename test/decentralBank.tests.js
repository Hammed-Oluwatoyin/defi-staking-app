...
contract("DecentralBank", ([owner, customer]) => {
  ...
  describe("Mock Tether Deployment", async () => {
    //first test: run assertion to check we have successful name matching
    it("matches name successfully", async () => {
      // allows us to bring in a description
  ...
  describe("Yield Farming", async () => {
    it("rewards tokens for staking", async () => {
      ...
      assert.equal(
        result.toString(),
        tokens("0"),
        "customer wallet balance after staking 100 tokens"
      );
      ...
      assert.equal(
        result.toString(),
        "false",
        "customer is NOT staking anymore"
      );
    });
  });
});
